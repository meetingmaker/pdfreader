...
class PDF extends PureComponent {
  constructor(props) {
    ...
    this.handleDispatchKeydownToIframe =
      this.handleDispatchKeydownToIframe.bind(this);
    this.handlePressEscape = this.handlePressEscape.bind(this);
    this.handleClickOnPopupContainer =
      this.handleClickOnPopupContainer.bind(this);
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevProps.downloadable !== this.props.downloadable ||
      ...
      this.iframe.contentWindow.postMessage(
        { message: "onResourceChanged", data: resource },
        host
      );
    }
  }
  ...
  goToPage(page) {
    ...
    const host = this.props.viewer.host;
    this.iframe.contentWindow.postMessage(
      { message: "onPageChanged", data: page },
      host
    );
  }
  ...
  startPresentation(data) {
    const host = this.props.viewer.host;
    this.iframe.contentWindow.postMessage(
      { message: "onPresentationStarted", data },
      host
    );
  }
  ...
  handleDispatchKeydownToIframe(event) {
    ...
    this.iframe.contentWindow.postMessage(
      { message: "onKeydown", data: { key, keyCode } },
      host
    );
  ...
  render() {
    if (this.props.popup) {
      return (
        ...
        >
          <div className={styles.popupContent}>{this.renderIframe()}</div>
        </div>
      );
    }
...
PDF.propTypes = {
  ...
  src: PropTypes.oneOfType([PropTypes.string, PropTypes.instanceOf(Object)])
    .isRequired,
